...
fn distance_along_line_to_point(line: &geo::LineString<f64>, point: &geo::Point<f64>) -> f64 {
    use geo::prelude::{HaversineDistance, EuclideanDistance};

    let mut total_distance = 0.0;
    let mut min_dist_to_segment = f64::MAX;
    let mut closest_point_on_segment = geo::Point::new(0.0, 0.0);
    let mut closest_segment_index = 0;

    let points: Vec<geo::Point<f64>> = line.points().map(|p| p.clone()).collect(); // Collect owned points
    ...
    for i in 0..points.len() - 1 {
        ...
        let segment_start = geo::Point::new(p1.x(), p1.y());
        let segment_end = geo::Point::new(p2.x(), p2.y());

        // Project point onto segment
        let segment_vec = geo::Point::new(segment_end.x() - segment_start.x(), segment_end.y() - segment_start.y());
        let point_vec = geo::Point::new(point.x() - segment_start.x(), point.y() - segment_start.y());
        ...
        let projected_point = geo::Point::new(projected_x, projected_y);
...
fn process_tracking_data(
    line_string_geojson: geojson::GeoJson,
    tracking_points_js: Vec<TrackingPointJs>,
) -> Result<ProcessedTrackingDataResult, String> {
    use geo::prelude::{HaversineLength, ClosestPoint};
    use geo::Closest;
    use geo::Point;

    let line_string_geo = match line_string_geojson {
        geojson::GeoJson::Feature(feature) => {
            if let Some(geojson::Geometry { value: geojson::Value::LineString(ls_geojson), .. }) = feature.geometry {
                geo::LineString::new(ls_geojson.into_iter().map(|pos| Coord { x: pos[0], y: pos[1] }).collect())
            } else {
                return Err("GeoJSON Feature does not contain a LineString geometry.".to_string());
            }
        },
        geojson::GeoJson::Geometry(geometry) => {
            if let geojson::Value::LineString(ls_geojson) = geometry.value {
                geo::LineString::new(ls_geojson.into_iter().map(|pos| Coord { x: pos[0], y: pos[1] }).collect())
            } else {
    ...
    for tp_js in tracking_points_js {
        ...
        let distance_km = match nearest_point_on_line {
            ...
            _ => 0.0, // Should not happen if line_string_geo is valid, or handle other Closest variants
        };
