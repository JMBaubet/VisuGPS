...
async fn generate_gpx_thumbnail(
    ...
    // Convertir la couleur Vuetify en hex (simplifié, nécessite une vraie conversion)
    // Pour l'instant, on utilise une couleur fixe ou on cherche une correspondance simple
    let gpx_color_hex = match color_gpx_vignette {
        "orange-darken-4" => "FF6F00", // Exemple
        _ => "FF6F00", // Couleur par défaut
    };
    ...
    if show_markers {
        ...
        if dist_meters <= distance_max_close {
            ...
            let color_hex = match couleur_depart_arrivee {
            ...
            let color_start_hex = match couleur_depart {
                "green-darken-2" => "4CAF50",
                _ => "4CAF50",
            };
            let color_end_hex = match couleur_arrivee {
                "red-darken-2" => "F44336",
                _ => "F44336",
            };
